...
import { class11Chapters, getChapterBySlug, getAdjacentChapters } from "@/lib/chapters";
...
export default async function ClassElevenChapterLayout({
  params,
  children,
}: {
  ...
  const basePath = `/class-11/${chapter.slug}`;
  const { prev, next } = getAdjacentChapters("11", chapter.slug);
      ...
      <section className="physics-grid bg-navy text-white">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-14 sm:py-20">
          ...
          <p className="mt-6 text-gold text-xs font-semibold uppercase tracking-[0.2em]">
            Class 11 &middot; Chapter {chapter.number}
          </p>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
          ...
          <div className="mt-12 rounded-lg bg-navy text-white p-8 sm:p-10 physics-grid text-center">
            ...
            <div className="mt-6 flex justify-center">
              <WhatsAppButton
                message={`Hi Ajay Sir, I have a doubt in Class 11 - ${chapter.name}.`}
              >
                Ask a doubt
              </WhatsAppButton>
            </div>
          ...
          <div className="mt-8">
            ...
            >
              <span aria-hidden="true">&larr;</span> Back to Class 11 chapters
            </Link>
          </div>
        </div>
      </section>
