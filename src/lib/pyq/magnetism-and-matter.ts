...
export const magnetismAndMatterPyq: ChapterPyq = {
  ...
  questions: [
    ...
    },
    {
      ...
      correctIndex: 3,
      solution:
        "[FLAGGED DISCREPANCY — read carefully.] The exact fraction/root symbols in the source scan were not legible enough to transcribe with full confidence, and I want to be transparent that my own derivation does not land on the option position the printed key indicates. My derivation: at equilibrium (θ=0), U=−mB (minimum). Work done rotating to θ=60°: W = U(60°)−U(0°) = −mBcos60°−(−mB) = mB(1−0.5) = mB/2, so mB=2W. Torque to hold the magnet at 60°: τ = mBsin60° = 2W×(√3/2) = √3·W — this is option 2 in my reconstructed list, not option 4 (2√3·W) that the printed key points to. correctIndex has been set to 3 to match the key per the rule of never silently overriding a printed answer, but this should be treated as an open discrepancy — likely caused by my inability to read the exact original option text — rather than a fully verified solution. Recommend re-deriving from a clearer scan of this specific figure before trusting this one with full confidence.",
    },
    {
      exam: "AIIMS 2016 [Q64]",
      question:
